	...
	for mod in dat("Mods"):Rows() do
		if condFunc(mod) then
			...
			if #orders > 0 then
				...
				if not mod.GenerationWeightTags[1] and mod.Tags[1] then
					out:write('tags = { ')
					for _, tag in ipairs(mod.Tags) do
						out:write('"', tag.Id, '", ')
					end
			...
			-- adjust final number or multiplier if it breaks
			constantCount = constantCount + 13 + #stats + #orders + (2 * #mod.SpawnWeights) + (3 * #mod.GenerationWeightTags) + 0
			if constantCount >= (65536 * 3.6) then
				print(outName.." exceeded constant limit, splitting file.")
				out:write('}')
				out:close()
				out = io.open(outName:gsub(".lua", ("_part"..tostring(fileSplitCount + 2)..".lua")), "w")
				out:write('-- This file is automatically generated, do not edit!\n')
				out:write('-- Item data (c) Grinding Gear Games\n\nreturn {\n')
				constantCount = 2
